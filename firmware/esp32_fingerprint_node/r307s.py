...
class R307Sensor:
    ...
    def connect(self):
        ...
        try:
            ...
            _sleep_ms(100)
            return self._cmd(_CMD_TEMPLATECOUNT) == OK
        except Exception:
            return False
    ...
    def _read_response(self, timeout_ms=3000):
        ...
        while _ticks_diff(_ticks_ms(), start) < timeout_ms:
            ...
            while index < len(buf) - 1:
                if buf[index] == 0xEF and buf[index + 1] == 0x01:
                    break
                index += 1
            else:
                _sleep_ms(10)
            ...
            if len(buf) - index < 9:
                _sleep_ms(10)
                continue
            packet_len = (buf[index + 7] << 8) | buf[index + 8]
            total = index + 9 + packet_len
            if len(buf) < total:
                _sleep_ms(10)
                continue
            confirm = buf[index + 9]
            extra = bytes(buf[index + 10: index + 9 + packet_len - 2])
    ...
    def get_template_count(self):
        if self._uart is None:
            return 0
        self._uart.write(self._build_packet(bytearray([_CMD_TEMPLATECOUNT])))
        code, extra = self._read_response()
        if code == OK and len(extra) >= 2:
